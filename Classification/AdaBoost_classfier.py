...
predicted = alfa[0] * algs[0].predict(np.c_[x.ravel(), y.ravel()]).reshape(x.shape)
for n in range(1, T):
    predicted += alfa[n] * algs[n].predict(np.c_[x.ravel(), y.ravel()]).reshape(x.shape)

plt.pcolormesh(x, y, predicted, cmap = 'spring', shading = 'auto')
